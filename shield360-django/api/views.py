...
from .models import Alert, ChatbotMessage, Module, Progress, Report, User
...
def update_preferences(request):
    ...
    user.preferences_high_contrast = prefs.get(
        "highContrast", user.preferences_high_contrast
    )
    user.save()
...
def analyze_sentiment(text):
    ...
    positive = any(
        w in t
        for w in ["happy", "good", "great", "fine", "okay", "well", "better", "excited"]
    )
    negative_hits = sum(
        w in t
        for w in [
...
def chatbot_history(request):
    items = ChatbotMessage.objects.filter(user=request.user).order_by("-created_at")[
        :50
    ]
...
def sos_send(request):
    ...
    return Response(
        {
            "success": True,
            "message": "SOS alert sent to guardian",
...
def sos_alerts(request):
    if request.user.role != "guardian":
        return Response({"message": "Access denied"}, status=403)
    alerts = (
        Alert.objects.filter(guardian=request.user)
        .select_related("user")
        .order_by("-created_at")
    )
...
def admin_analytics(request):
    ...
    reports_by_status = list(
        Report.objects.values('_id=models.F("status")')
        .annotate(count=Count("id"))
        .values("status", "count")
    )
    alerts_by_type = list(
        Alert.objects.values('_id=models.F("type")')
        .annotate(count=Count("id"))
        .values("type", "count")
    )
